<template>
	<view class="index_container">
				...
				<view class="index-list" v-for="(item,index) in proList">
					...
					<view class="index-uinn">
						...
						<view class="index_collect" style="padding-top: 5px; width: 100%;">
							...
							<button type="warn" @click="selectCustomer(item)" style="margin: 0 auto; width: 110px; border-radius: 5px;; height: 40px; line-height: 40px; font-size: 14px;" class="cart-bot-right">立即购买</button>
						</view>
					</view>
				</view>
			</view>
		...
		<view class="cart-cust-dialog" v-show="isShow">
			...
			<view class="login-list ub-ac" style="padding-top: 10px;">
					...
					for(let i=0;i<this.secondLevel.length;i++){
						if(this.secondLevel[i].s_level3.length > 0){
							if(i == index){
								...
								for(let s=0;s<this.secondLevel[i].s_level3.length;s++){
									this.secondLevel[i].s_level3[s].ischecked = false;
								}
							}
						}
					}
