				...
				<view style="padding-top: 10px;">
					...
					<view v-for="(items, index) in secondLevel" :key="index">
						<view v-if="items.s_level3.length > 0" @click="xgetCodePro(items.s_mb001, index, true)" class="pl15" v-bind:class="{ active: items.isChecd }">
							<view class="handle">点击查看</view>{{index + 1}}: {{items.s_level2}}
							<view style="font-weight: bold; padding-left: 70px;">{{items.spec}}</view>
						...
						<view @click="xgetCodePro(e.s_mb001, index, false)" class="viewleve1 pl25" v-for="(e, i) in items.s_level3" v-show="e.isshow">
							<view>{{index+1}}-{{i + 1}}: {{e.s_level3}}</view>
							<view style="font-weight: bold;">{{e.spec}}</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getCar(){
				...
				this_.$http.httpTokenRequest({
					...
					console.log('原车代码');
					console.log(res)
					//console.log(res.data[0].s_car_code)
					//console.log(res.data[0].s_level2)
					//请求成功
					uni.hideLoading();
					console.log("二级");
					console.log(res.data[0].s_level2);
		...
		watch: {
			secondLevel(){
				console.log(this.secondLevel.length)
